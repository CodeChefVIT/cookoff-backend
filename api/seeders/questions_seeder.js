...
mongoose.connect("mongodb://localhost:27017/mydb").then(() => console.log("Connection Successfull")).catch("Error connecting");
...
const testQuestions = [
    {
        ...
        inputFormat: ["Line 1 : N, the total number of potential customers.", "Lines 2 to N+1: Each line has the budget of a potential customer."],
        outputFormat: [`The output consists of a single integer, the maximum possible revenue you can earn from selling your app
        `],
        sampleTestInput: `4
        30
        20
...
const addData = async () => {
    ...
    for (let i = 0; i < insertedQuestions.length; i++) {
        testQuestions[i].testCases.forEach((tc) => tc.question = insertedQuestions[i]._id);
